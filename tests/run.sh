...
# ========================================================================
# 145-149b. Pair stall watchdog. The pump (lib/python/pair_pump.py) runs a
# single delivery loop that (a) delivers a steer the instant the turn it landed
# in completes, (b) exits when its downstream (claude) closes, and (c) reaps the
# child's process group -- isolated under exec_setsid.py -- if the child log
# freezes past CEREBRO_PAIR_STALL, writing both a synthetic `stalled` result and
# a `.stalled` sidecar. The wrapper turns the sidecar into a bounded
# resume-with-backoff loop. Tests 145-148 drive the pump directly with a fake
# claude; 149/149b drive a real `cerebro execute --pair` with a stub that stalls
# on demand. Gated on python3.
# ========================================================================
if command -v python3 >/dev/null 2>&1; then
  PYLIB="$here/../lib/python"

  # ppump_wait <bg-pid> <secs> -- wait for a backgrounded pipeline up to <secs>;
  # on timeout kill it and any leaked pump so a regression cannot hang the suite.
  # Returns the job's exit code, or 124 on timeout.
  ppump_wait() {
    ...
    kill "$pid" 2>/dev/null
    pkill -f 'pair_pump.py' 2>/dev/null || true
    wait "$pid" 2>/dev/null
    return 124
  }

  # --- 145. a steer that lands mid-turn is delivered once the turn completes,
  # and recorded to .steering.md only after a successful emit. ---
  P145_LOG="$WORKDIR/p145.jsonl"; : > "$P145_LOG"
  P145_FIFO="$WORKDIR/p145.steer.fifo"; rm -f "$P145_FIFO"; mkfifo "$P145_FIFO"
  P145_STEER="$WORKDIR/p145.steering.md"; : > "$P145_STEER"
  P145_TURNS="$WORKDIR/p145.turns"; : > "$P145_TURNS"
  P145_MARK="$WORKDIR/p145.mark"; rm -f "$P145_MARK"
  cat > "$WORKDIR/fake145.sh" <<'FAKE'
#!/usr/bin/env bash
clog="$1"; turns="$2"; marker="$3"; first=1
while IFS= read -r line; do
  printf '%s\n' "$line" >> "$turns"
  if (( first )); then : > "$marker"; sleep 1; first=0; fi   # hold turn 1 open
  printf '%s\n' '{"type":"assistant","message":{"role":"assistant","content":[{"type":"text","text":"working"}]}}' >> "$clog"
  printf '%s\n' '{"type":"result","subtype":"success","result":"ok"}' >> "$clog"
done
FAKE
  ( printf 'do the work' \
  ...
  ppump_wait "$p145" 15 >/dev/null 2>&1; r145=$?
  wait "$s145" 2>/dev/null
  if [[ $r145 -ne 124 ]] && grep -q 'use a hashmap 145' "$P145_TURNS" \
     && grep -q 'use a hashmap 145' "$P145_STEER"; then
    printf 'PASS  145  pump delivers a mid-turn steer after the turn + records it\n'; pass=$((pass + 1))
  else
    printf 'FAIL  145  mid-turn steer [rc=%d turns=%s]\n' "$r145" "$(cat "$P145_TURNS")"; fail=$((fail + 1))
    failures+=("145 mid-turn steer :: rc=$r145")
  fi
  rm -f "$P145_FIFO"

  # --- 146. once the pump finishes (idle window elapses) a later steer is
  # undelivered (steer_send exits 3) and the fifo reports not-live. ---
  P146_LOG="$WORKDIR/p146.jsonl"; : > "$P146_LOG"
  P146_FIFO="$WORKDIR/p146.steer.fifo"; rm -f "$P146_FIFO"; mkfifo "$P146_FIFO"
  cat > "$WORKDIR/fake146.sh" <<'FAKE'
#!/usr/bin/env bash
clog="$1"
while IFS= read -r line; do
  printf '%s\n' '{"type":"result","subtype":"success","result":"ok"}' >> "$clog"
...
FAKE
  ...
  python3 "$PYLIB/fifo_live.py" "$P146_FIFO" >/dev/null 2>&1; flc146=$?
  if [[ $r146 -ne 124 && $src146 -eq 3 && $flc146 -ne 0 ]]; then
    printf 'PASS  146  post-finish steer is undelivered (exit 3) + fifo dead\n'; pass=$((pass + 1))
  else
    printf 'FAIL  146  post-finish steer [rc=%d steer=%d live=%d]\n' "$r146" "$src146" "$flc146"; fail=$((fail + 1))
    failures+=("146 post-finish steer :: steer=$src146 live=$flc146")
  fi
  rm -f "$P146_FIFO"

  # --- 147. the pump releases the fifo when its downstream (claude) closes,
  # even with no turn ever completing (idle/stall set far out of reach). ---
  P147_LOG="$WORKDIR/p147.jsonl"; : > "$P147_LOG"
  P147_FIFO="$WORKDIR/p147.steer.fifo"; rm -f "$P147_FIFO"; mkfifo "$P147_FIFO"
  cat > "$WORKDIR/fake147.sh" <<'FAKE'
#!/usr/bin/env bash
...
FAKE
  ...
  if [[ $r147 -ne 124 && $flc147 -ne 0 ]]; then
    printf 'PASS  147  pump releases the fifo on downstream close\n'; pass=$((pass + 1))
  else
    printf 'FAIL  147  downstream close [rc=%d live=%d]\n' "$r147" "$flc147"; fail=$((fail + 1))
    failures+=("147 downstream close :: rc=$r147 live=$flc147")
  fi
  rm -f "$P147_FIFO"

  # --- 148. STALL: the pump reaps ONLY the child's group (via exec_setsid's
  # pgid), writes both stall markers, and releases the fifo. ---
  P148_LOG="$WORKDIR/p148.jsonl"; : > "$P148_LOG"
  P148_FIFO="$WORKDIR/p148.steer.fifo"; rm -f "$P148_FIFO"; mkfifo "$P148_FIFO"
  P148_PGID="$WORKDIR/p148.pgid"; rm -f "$P148_PGID"
  P148_SPID="$WORKDIR/p148.sentinel"; rm -f "$P148_SPID"
  cat > "$WORKDIR/fake148.sh" <<'FAKE'
#!/usr/bin/env bash
clog="$1"; spid="$2"
sleep 300 &                 # sentinel in the child's group
echo $! > "$spid"
printf '%s\n' '{"type":"assistant","message":{"role":"assistant","content":[{"type":"text","text":"thinking"}]}}' >> "$clog"
...
FAKE
  ...
  salive=0; [[ -n "$sentinel" ]] && kill -0 "$sentinel" 2>/dev/null && salive=1
  python3 "$PYLIB/fifo_live.py" "$P148_FIFO" >/dev/null 2>&1; flc148=$?
  if [[ $r148 -ne 124 && $salive -eq 0 ]] && grep -q '"subtype":"stalled"' "$P148_LOG" \
     && [[ -e "${P148_LOG%.jsonl}.stalled" && $flc148 -ne 0 ]]; then
    printf 'PASS  148  stall reaps the child group + writes both markers + frees fifo\n'; pass=$((pass + 1))
  else
    printf 'FAIL  148  stall reap [rc=%d sentinel_alive=%d live=%d]\n' "$r148" "$salive" "$flc148"; fail=$((fail + 1))
    failures+=("148 stall reap :: sentinel_alive=$salive live=$flc148")
  fi
  ...
  # then the run finishes 0 with a success result and no marker left behind. ---
  STALL_STUB_DIR="$WORKDIR/claude-stall-stub"; mkdir -p "$STALL_STUB_DIR"
  STALL_ARGV_LOG="$WORKDIR/stall-argv.log"; : > "$STALL_ARGV_LOG"
  cat > "$STALL_STUB_DIR/claude" <<EOF
#!/usr/bin/env bash
printf '%s\n' "\$*" >> "$STALL_ARGV_LOG"
prev=""; sid=""; resume=0
for a in "\$@"; do
  [[ "\$prev" == "--session-id" || "\$prev" == "--resume" ]] && sid="\$a"
  [[ "\$a" == "--resume" ]] && resume=1
  prev="\$a"
done
[[ -n "\$sid" ]] || sid="STALL-NOPIN"
printf '{"type":"system","subtype":"init","session_id":"%s"}\n' "\$sid"
if (( resume )); then
  while IFS= read -r _l; do
    printf '%s\n' '{"type":"result","subtype":"success","result":"resumed-ok"}'
  done
else
  cat >/dev/null   # first launch: emit no result, freeze -> stall
fi
exit 0
EOF
  chmod +x "$STALL_STUB_DIR/claude"
  if [[ -x "$STALL_STUB_DIR/claude" ]]; then
    WSESS="wrap-stall-session"; WDIR="$CEREBRO_HOME/sessions/$WSESS"
    mkdir -p "$WDIR/children" "$WDIR/plans"; : > "$WDIR/transcript.jsonl"
    ( env PATH="$STALL_STUB_DIR:$PATH" CEREBRO_SESSION_ID="$WSESS" \
        CEREBRO_PAIR_IDLE=1 CEREBRO_PAIR_STALL=2 CEREBRO_PAIR_STALL_BACKOFF=0 \
        "$CEREBRO_BIN" execute "$REPO" --prompt "do the work" --branch feat/restart --pair \
        >"$WORKDIR/w149.out" 2>"$WORKDIR/w149.err"; echo $? > "$WORKDIR/w149.rc" ) & w149=$!
    ppump_wait "$w149" 40 >/dev/null 2>&1; tw149=$?
    r149="$(cat "$WORKDIR/w149.rc" 2>/dev/null)"
    clog149="$(tail -1 "$WORKDIR/w149.out" 2>/dev/null)"
    nres149="$(grep -c -- '--resume' "$STALL_ARGV_LOG" 2>/dev/null)"
    if [[ "$tw149" -ne 124 && "$r149" == "0" && "$nres149" == "1" ]] \
       && grep -q '"what":"pair_stall_restart"' "$WDIR/transcript.jsonl" \
       && grep -q 'attempt=1' "$WDIR/transcript.jsonl" \
       && grep -q 'backoff=0s' "$WDIR/transcript.jsonl" \
       && [[ -s "$clog149" ]] && grep -q '"subtype":"success"' "$clog149" \
       && [[ ! -e "${clog149%.jsonl}.stalled" ]]; then
      printf 'PASS  149  stall auto-restart resumes once (backoff applied) then finishes\n'; pass=$((pass + 1))
    else
      printf 'FAIL  149  auto-restart [tw=%d rc=%s resumes=%s clog=%s]\n' "$tw149" "$r149" "$nres149" "$clog149"; fail=$((fail + 1))
      failures+=("149 auto-restart :: rc=$r149 resumes=$nres149")
    fi

    # --- 149b. give-up after CEREBRO_PAIR_STALL_RETRIES: a stub that stalls on
    # every launch exits non-zero, logs pair_stall_giveup, surfaces a "remains
    # resumable" message, clears the marker, and leaves the child status=running
    # (no child_store_done) so it stays resumable. ---
    STALL2_STUB_DIR="$WORKDIR/claude-stall2-stub"; mkdir -p "$STALL2_STUB_DIR"
    STALL2_ARGV_LOG="$WORKDIR/stall2-argv.log"; : > "$STALL2_ARGV_LOG"
    cat > "$STALL2_STUB_DIR/claude" <<EOF
#!/usr/bin/env bash
printf '%s\n' "\$*" >> "$STALL2_ARGV_LOG"
prev=""; sid=""
for a in "\$@"; do
...
[[ -n "\$sid" ]] || sid="STALL2-NOPIN"
printf '{"type":"system","subtype":"init","session_id":"%s"}\n' "\$sid"
cat >/dev/null   # every launch stalls
exit 0
EOF
    chmod +x "$STALL2_STUB_DIR/claude"
    W2SESS="wrap-giveup-session"; W2DIR="$CEREBRO_HOME/sessions/$W2SESS"
    mkdir -p "$W2DIR/children" "$W2DIR/plans"; : > "$W2DIR/transcript.jsonl"
    ( env PATH="$STALL2_STUB_DIR:$PATH" CEREBRO_SESSION_ID="$W2SESS" \
        CEREBRO_PAIR_IDLE=1 CEREBRO_PAIR_STALL=2 CEREBRO_PAIR_STALL_RETRIES=1 CEREBRO_PAIR_STALL_BACKOFF=0 \
        "$CEREBRO_BIN" execute "$REPO" --prompt "do the work" --branch feat/giveup --pair \
        >"$WORKDIR/w150.out" 2>"$WORKDIR/w150.err"; echo $? > "$WORKDIR/w150.rc" ) & w150=$!
    ppump_wait "$w150" 40 >/dev/null 2>&1; tw150=$?
    r150="$(cat "$WORKDIR/w150.rc" 2>/dev/null)"
    err150="$(cat "$WORKDIR/w150.err" 2>/dev/null)"
    st150="$(jq -r '.[].status' "$W2DIR/child-sessions.json" 2>/dev/null)"
    leftover150="$(ls "$W2DIR"/children/*.stalled 2>/dev/null)"
    if [[ "$tw150" -ne 124 && -n "$r150" && "$r150" != "0" ]] \
       && grep -q '"what":"pair_stall_giveup"' "$W2DIR/transcript.jsonl" \
       && [[ "$err150" == *"remains resumable"* ]] \
       && [[ -z "$leftover150" && "$st150" == "running" ]]; then
      printf 'PASS  149b  stall give-up surfaces resumable child + keeps it running\n'; pass=$((pass + 1))
    else
      printf 'FAIL  149b  give-up [tw=%d rc=%s status=%s leftover=%s]\n' "$tw150" "$r150" "$st150" "$leftover150"; fail=$((fail + 1))
      failures+=("149b give-up :: rc=$r150 status=$st150")
    fi
  else
    for t in 149 149b; do
      printf 'SKIP  %s  pair stall wrapper (stub unavailable)\n' "$t"
    done
  fi
else
