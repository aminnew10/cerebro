...
export const CerebroSessionBinding = async () => {
  ...
  const recordSid = (sid) => {
    ...
    try {
      const scriptPath = path.join(process.env.CEREBRO_LIB_DIR || path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../.."), "python", "update_ports_sid.py")
      if (fs.existsSync(scriptPath)) {
        execSync(`python3 "${scriptPath}" "${dir}" "${sid}"`, { stdio: "ignore" })
      }
    } catch {}
  }
