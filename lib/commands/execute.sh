...
cmd_execute() {
  ...
  # CEREBRO_PAIR_STALL is reaped by the pump, which leaves a .stalled sidecar
  # and a non-zero rc; we relaunch it with --resume on a fresh fifo up to
  # CEREBRO_PAIR_STALL_RETRIES times (exponential backoff), then give up
  # cleanly while leaving the child resumable. The provider-rejected stale
  # resume fallback stays the INNER step, gated so a stall never triggers it.
  local stall_n=0
  while :; do
    local run_opts=("${opts[@]}")
    ...
    # Stale fallback (INNER): a resume can be rejected up front because the
    # provider GC'd or no longer recognizes the stored conversation. Retry once
    # fresh (no --resume) ONLY when the resumed run did no work before failing
    # -- it never emitted a session/init event, so id_capture stays empty and
    # nothing was committed or edited -- and NOT when the child stalled (that is
    # the OUTER loop's job). If the resumed child DID start a session
    ...
    # work already done. Such a failure is fatal and is handled below.
    ...
    # Bounded stall-restart (OUTER) with exponential backoff.
    if (( pair )) && pair_stalled "$child_log"; then
      if (( stall_n < ${CEREBRO_PAIR_STALL_RETRIES:-2} )); then
        stall_n=$((stall_n + 1))
        pair_stall_backoff "$stall_n"                               # wait base*2^(n-1)
        pair_stall_clear "$child_log"                               # consume marker before relaunch
        pair_begin execute "$repo" "$new_branch" "$child_log" "$PAIR_SID"
        prior="$PAIR_SID"                                           # relaunch with --resume <pinned id>
        continue                                                    # fresh pump run -> stall timer resets
      fi
