...
cmd_apply_review() {
  ...
  while :; do
    ...
    # resumed run never started -- no init event, so id_capture is empty and
    # nothing was mutated -- AND did not stall. A resume that started then
    # failed is fatal (handled below); a fresh re-run would duplicate the work
    # it already applied.
